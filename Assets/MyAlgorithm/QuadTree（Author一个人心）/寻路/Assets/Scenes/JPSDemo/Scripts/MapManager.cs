    ...
    public class MapManager : MonoBehaviour, IGrid {
        ...
        private void BuildUpdate() {
            if (Input.GetMouseButtonDown(0)) {
                ...
                if (hit.collider != null && hit.collider.TryGetComponent(out JPSCell cell)) {
                    switch (type) {
                    case SettingType.Obstacle:
                        if (cell == start) {
                            start = null;
        ...
        private void SearchUpdate() {
            if (Input.GetKeyDown(KeyCode.Space)) {
                if (isFirsetSearch) {
                    ...
                    enumerator = jpsTest.IFindStep(start.Pos, end.Pos).GetEnumerator();
        ...
        public void SetJump(in Vector2Int pos) {
            cells[pos.x, pos.y].SetJumpPoint();
        }

        public void SetSearch(in Vector2Int pos) {
            cells[pos.x, pos.y].SetSearch();
        }
        ...
        private void BuildMap() {
            ...
            cells = new JPSCell[mapSize.x, mapSize.y];
            ...
            for (int i = 0; i < mapSize.x; i++) {
                for (int j = 0; j < mapSize.y; j++) {
                    ...
                    cells[i, j] = go.GetComponent<JPSCell>();
